...
async def generate_flashcards_async(job_id, project_id, params):
    try:
        update_job(job_id, status="running", progress=15, message="Extracting concepts…")
        db = SessionLocal()
        t = db.query(Transcript).filter(Transcript.project_id==project_id).first()
        db.close()
        if not t: update_job(job_id, status="failed", error="Transcript not ready"); return
        from app.services.quiz_generator_v2 import extract_concepts, _filter_transcript, _find_definition
        filtered = _filter_transcript(t.full_text)
        if len(filtered.split()) < 50: filtered = t.full_text
        update_job(job_id, progress=40, message="Building flashcards from concepts…")
        concepts = extract_concepts(filtered, top_n=params.get("num_cards",20))
        cards = []
        for concept in concepts:
            defn = _find_definition(filtered, concept)
            if not defn or len(defn.split()) < 6: continue
            back = defn.strip()
            if len(back) > 250: back = back[:247] + "…"
            cards.append({"id":str(uuid.uuid4()),"front":f"What is '{concept}'?",
                          "back":back,"concept":concept,"difficulty":"medium"})
            if len(cards) >= params.get("num_cards",20): break
        if not cards:
            update_job(job_id, status="failed",
                       error="Could not extract concepts. Ensure transcript is available."); return
        update_job(job_id, progress=80, message="Saving flashcards…")
        db = SessionLocal()
        db.query(StudyAsset).filter(StudyAsset.project_id==project_id,
                                     StudyAsset.asset_type=="flashcard").delete()
        db.add(StudyAsset(id=str(uuid.uuid4()),project_id=project_id,
